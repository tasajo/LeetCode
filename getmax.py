...
class Solution:
    def getMaximumGenerated(self, n: int) -> int:
        ...
        if n == 0:
            # Quick response for n == 0
            return 0
        
        maximum = 1
        
        # update for each item from 2 to n
        for j in range(2, n+1):
            ...
            if j % 2 == 0:
                ...
                i = (j - 1) // 2
                table[j] = table[i] + table[i+1]
            
            # update maximum value
            maximum = max(maximum, table[j])
            
        return maximum
    
    print("The maximum in the array is", getMaximumGenerated("", 7))
